...
export function TitleScreenAbl({ onStart, onHowTo }: TitleScreenProps) {
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      // stats: inter-frame interval (rAF cadence) + start of the timed region
      if (prevTs) stats.iv.push(now - prevTs)
      prevTs = now
      ...
      if (bctx && cellBuf && (F.cells || F.glow || F.chroma || F.composite)) {
        ...
        if (F.chroma && focus < 0.99) {
          ctx.globalAlpha = 0.5
          ctx.drawImage(cellBuf, split, 0, W, H)
          ctx.drawImage(cellBuf, -split, 0, W, H)
          ctx.globalAlpha = 1
        }
        if (F.composite) ctx.drawImage(cellBuf, 0, 0, W, H)
        ctx.globalCompositeOperation = 'source-over'
      }

      // the pipette tip: a bright bloom that follows the cursor
      if (F.pointer && pointerX >= 0 && sprites.white) {
        ctx.globalCompositeOperation = 'lighter'
        const s2 = cell * 3.4
      ...
      // ── eyepiece chrome ────────────────────────────────────────────────────
      if (F.reticle) drawReticle(ctx, W, H, now, focus, reduced)
      if (F.marquee) drawMarquee(ctx, marqueePts, now, reduced)
