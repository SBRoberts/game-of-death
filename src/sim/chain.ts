 ...
 * matter (the ash payout lives in the meta layer). Storm deaths are excluded
 * upstream (combatDeaths), and the tracker only arms after a player action, so
 * a chain is legibly the player's doing rather than ambient grind or the ring.
 */
...
export const CHAIN_TIERS: readonly ChainTier[] = [
  ...
  { name: 'ROUT', at: 25 },
  { name: 'MASSACRE', at: 60 },
  { name: 'CATACLYSM', at: 120 },
  { name: 'EXTINCTION EVENT', at: 250 },
]
...
export const CHAIN_FLOOR = 4 // per-gen spike above baseline that keeps a chain alive
export const CHAIN_SPIKE_CAP = 35 // max one gen can contribute (a single wipe can't insta-max)
export const CHAIN_ARM_GENS = 20 // window after a player action during which chains count
